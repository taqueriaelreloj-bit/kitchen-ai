...
describe('precision 2D plan snapping',()=>{
  ...
  test('joins cabinet edges and returns an alignment guide',()=>{
    ...
    const result=snapPlanObject(moving,{x:132.5,y:101.5},[target,moving],{grid:false,toleranceIn:4});
  ...
  test('aligns object centers',()=>{
    ...
    const proposedX=targetBounds.centerX-moving.widthIn/2+2;
    const result=snapPlanObject(moving,{x:proposedX,y:320},[target],{grid:false,toleranceIn:3});
  ...
  test('uses rotated footprint for quarter-turn cabinets',()=>{
    ...
    expect(bounds.bottom-bounds.top).toBe(36);
  });
