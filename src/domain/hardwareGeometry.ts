...
import { CabinetHardware, EditorObject, normalizeCabinetHardware } from './editor';
...
const sizeIn=(hardware:CabinetHardware,object:EditorObject)=>{
  if(hardware.size==='Custom Size')return hardware.customSizeIn??5;
...
const hardwarePosition=(hardware:CabinetHardware,object:EditorObject)=>{
  const limitX=Math.max(0,object.widthIn/2-2);
  const limitY=Math.max(0,object.heightIn/2-2);
  if(hardware.position==='Upper Corner')return {x:limitX,y:limitY};
  if(hardware.position==='Lower Corner')return {x:limitX,y:-limitY};
  if(hardware.position==='Edge Mounted')return {x:0,y:limitY};
  if(hardware.position==='Vertical')return {x:Math.min(limitX,Math.max(0,object.widthIn*.32)),y:0};
  if(hardware.position==='Horizontal')return {x:0,y:Math.min(limitY,Math.max(0,object.heightIn*.18))};
  if(hardware.position==='Custom Position')return {
    x:clamp(hardware.customOffsetXIn??0,-limitX,limitX),
    y:clamp(hardware.customOffsetYIn??0,-limitY,limitY),
  };
  return {x:0,y:0};
};
...
export function hardwareGeometry(object:EditorObject):HardwareGeometry|undefined{
  ...
  if(!object.hardware||hardware.style==='No Hardware')return undefined;
  ...
  const maximumSpan=Math.max(2,(vertical?object.heightIn:object.widthIn)*.72);
  const length=Math.min(Math.max(2,sizeIn(hardware,object)),maximumSpan);
  const pos=hardwarePosition(hardware,object);
  ...
  switch(hardware.style){
    ...
    case 'Tab Pull':
    case 'Finger Pull': {
      const edgeY=hardware.position==='Custom Position'?pos.y:Math.max(pos.y,object.heightIn*.48);
      parts=[part('edge',pos.x,edgeY,-.45,Math.min(length,object.widthIn*.55),.45,1.1)];
      break;
    }
    case 'T-Bar Pull':
