...
export function parseProject(serialized: string): EditorProject | undefined {
  try { return migrateProject(JSON.parse(serialized)); }
  catch { return undefined; }
}

export function projectFileName(project: EditorProject): string {
...
export function summarizeProject(project: EditorProject): ProjectExportSummary {
  return {
    ...
    appliances: project.objects.filter(object => object.kind === 'appliance').length,
...
export function projectScheduleCsv(project: EditorProject): string {
  const header = ['ID','Category','Name','Width (in)','Height (in)','Depth (in)','Material','Finish','Notes'];
  const rows = projectSchedule(project).map(row => [row.id,row.category,row.name,row.widthIn,row.heightIn,row.depthIn,row.material,row.finish,row.notes]);
