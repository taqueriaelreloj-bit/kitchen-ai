...
export function cabinetFinishType(object:EditorObject):CabinetFinishType{
  ...
  return(FINISH_TYPES as readonly string[]).includes(candidate)?candidate as CabinetFinishType:(finish.category==='wood'?'Natural':'Satin');
}
...
function resolveApplianceMaterial(object:EditorObject):PBRMaterialDescriptor{
  ...
  return{
    ...
    metalness:stainless||name.includes('metal')?.9:black?.65:.35,
    roughness:brushed?.46:name.includes('matte')?.8:name.includes('gloss')?.18:.32,
    opacity:1,
    transparent:false,
    textureKey:brushed?'brushed-stainless':undefined,
    normalMapKey:brushed?'brushed-normal':undefined,
    textureScale:{x:1,y:1},
    category:stainless||name.includes('metal')?'metal':white||black?'paint':'generic',
  };
}
