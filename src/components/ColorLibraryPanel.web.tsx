...
export function ColorLibraryPanel({ project, selected, apply }: { project: EditorProject; selected?: EditorObject; apply: (project: EditorProject) => void }) {
  ...
  const wallPaints = useMemo(() => WALL_PAINTS.filter(item => !q || item.displayName.toLowerCase().includes(q) || item.category.toLowerCase().includes(q)), [q]);
  const cabinetFinishes = useMemo(() => CABINET_FINISHES.filter(item => !q || item.displayName.toLowerCase().includes(q) || item.category.toLowerCase().includes(q)), [q]);
