...
function compileShader(gl:WebGLRenderingContext,type:number,source:string){const shader=gl.createShader(type);if(!shader)throw new Error('Unable to create WebGL shader.');gl.shaderSource(shader,source);gl.compileShader(shader);if(!gl.getShaderParameter(shader,gl.COMPILE_STATUS)){const message=gl.getShaderInfoLog(shader)??'Shader compile error';gl.deleteShader(shader);throw new Error(message);}return shader;}
function createProgram(gl:WebGLRenderingContext){const vertex=compileShader(gl,gl.VERTEX_SHADER,VERTEX_SHADER),fragment=compileShader(gl,gl.FRAGMENT_SHADER,FRAGMENT_SHADER),program=gl.createProgram();if(!program)throw new Error('Unable to create WebGL program.');gl.attachShader(program,vertex);gl.attachShader(program,fragment);gl.linkProgram(program);gl.deleteShader(vertex);gl.deleteShader(fragment);if(!gl.getProgramParameter(program,gl.LINK_STATUS)){const message=gl.getProgramInfoLog(program)??'WebGL link error';gl.deleteProgram(program);throw new Error(message);}return program;}
...
export function WebGLViewport({project,preview}:{project:EditorProject;preview:(project:EditorProject)=>void}){
  ...
  useEffect(()=>{
    ...
    try{
      ...
      if(!modelLocation||!vpLocation||!colorLocation||!roughLocation||!metalLocation||!eyeLocation)throw new Error('The 3D shader uniforms could not be initialized.');
      const dpr=Math.min(window.devicePixelRatio||1,2),width=viewport.width,height=viewport.height;
      canvas.width=Math.max(1,Math.floor(width*dpr));canvas.height=Math.max(1,Math.floor(height*dpr));
      gl.viewport(0,0,canvas.width,canvas.height);gl.clearColor(.88,.9,.89,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);

      const camera=project.camera3d,yaw=camera.yaw*Math.PI/180,pitch=camera.pitch*Math.PI/180,distance=Math.max(5,camera.distance/24);
      ...
      const viewProjection=multiplyMat4(perspectiveMatrix(Math.PI/4,canvas.width/canvas.height,near,far),lookAtMatrix(eye,target));
      gl.uniformMatrix4fv(vpLocation,false,new Float32Array(viewProjection));gl.uniform3fv(eyeLocation,new Float32Array(eye));
      hits.current.clear();
      for(const box of boxes){
        const model=multiplyMat4(translationMatrix(...box.center),multiplyMat4(rotationYMatrix(box.rotationY),scaleMatrix(...box.size)));
        ...
        gl.uniform1f(roughLocation,box.roughness);gl.uniform1f(metalLocation,box.metalness);gl.drawArrays(gl.TRIANGLES,0,36);
        if(box.sourceId&&!hits.current.has(box.sourceId)){
          const clip=transformPoint(viewProjection,box.center);
          if(clip[3]>.0001){const nx=clip[0]/clip[3],ny=clip[1]/clip[3];if(Number.isFinite(nx)&&Number.isFinite(ny)&&Math.abs(nx)<=1.25&&Math.abs(ny)<=1.25)hits.current.set(box.sourceId,{x:(nx*.5+.5)*width,y:(1-(ny*.5+.5))*height});}
        }
      }
      setRenderError(current=>current?undefined:current);
    }catch(error){setRenderError(error instanceof Error?error.message:'Kitchen AI could not render this 3D scene.');}
    return()=>{if(buffer)gl.deleteBuffer(buffer);if(program)gl.deleteProgram(program);};
  },[boxes,project.camera3d,project.selectedId,viewport]);

  const pick=(event:any)=>{const rect=event.currentTarget.getBoundingClientRect(),x=event.clientX-rect.left,y=event.clientY-rect.top;let id:string|undefined,best=44;for(const[candidate,point]of hits.current){const distance=Math.hypot(point.x-x,point.y-y);if(distance<best){best=distance;id=candidate;}}return id;};
  const panScale=Math.max(.12,project.camera3d.distance/Math.max(360,Math.min(viewport.width,viewport.height))*.75);
  return <View style={s.root}>
    <canvas ref={canvasRef} aria-label="3D kitchen viewport" style={{width:'100%',height:'100%',display:'block',cursor:drag.current.active?'grabbing':'grab'}} onContextMenu={event=>event.preventDefault()} onWheel={event=>{event.preventDefault();preview({...project,camera3d:{...project.camera3d,distance:Math.max(120,Math.min(1100,project.camera3d.distance+(event.deltaY>0?32:-32)))}});}} onMouseDown={event=>{drag.current={x:event.clientX,y:event.clientY,yaw:project.camera3d.yaw,pitch:project.camera3d.pitch,tx:project.camera3d.target.x,ty:project.camera3d.target.y,mode:event.button===1||event.button===2?'pan':'orbit',active:true,moved:false};}} onMouseMove={event=>{if(!drag.current.active)return;const dx=event.clientX-drag.current.x,dy=event.clientY-drag.current.y;if(Math.abs(dx)+Math.abs(dy)>3)drag.current.moved=true;if(drag.current.mode==='orbit')preview({...project,camera3d:{...project.camera3d,yaw:drag.current.yaw+dx*.28,pitch:Math.max(8,Math.min(76,drag.current.pitch+dy*.2))}});else preview({...project,camera3d:{...project.camera3d,target:{x:drag.current.tx-dx*panScale,y:drag.current.ty+dy*panScale}}});}} onMouseUp={event=>{drag.current.active=false;if(!drag.current.moved)preview({...project,selectedId:pick(event)});}} onMouseLeave={()=>{drag.current.active=false;}} onDoubleClick={event=>{const id=pick(event),object=project.objects.find(item=>item.id===id);if(object){const objectSize=Math.max(object.widthIn,object.depthIn,object.heightIn);preview({...project,selectedId:object.id,camera3d:{...project.camera3d,target:{x:object.x+object.widthIn/2,y:object.y+object.depthIn/2},distance:Math.max(120,Math.min(1100,objectSize*4.2))}});}}/>
    {renderError&&<View pointerEvents="none" style={s.error}><Text style={s.errorTitle}>3D view needs attention</Text><Text style={s.errorText}>{renderError}</Text><Text style={s.errorHelp}>The editable 2D plan remains available. Update the graphics driver or enable browser hardware acceleration, then reopen 3D.</Text></View>}
  </View>;
}

const s=StyleSheet.create({root:{flex:1,minHeight:0,position:'relative',backgroundColor:'#E0E5E3'},error:{position:'absolute',left:20,right:20,top:20,borderRadius:12,borderWidth:1,borderColor:'#C59E75',backgroundColor:'rgba(255,250,244,.96)',padding:14},errorTitle:{fontSize:15,fontWeight:'900',color:'#6D3D20'},errorText:{fontSize:12,lineHeight:18,fontWeight:'700',color:'#704F3C',marginTop:4},errorHelp:{fontSize:11,lineHeight:16,color:'#725E52',marginTop:5}});
