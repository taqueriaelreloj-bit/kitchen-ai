...
export function Workspace2D({project,preview,apply,commitHistory}:{project:EditorProject;preview:(project:EditorProject)=>void;apply:Apply;commitHistory:(snapshot:EditorProject,finalProject:EditorProject)=>void}){
  ...
  const webProps={
    ...
    onMouseMove:(event:any)=>{if(pan.current.active){preview({...project,view2d:{...project.view2d,pan:{x:pan.current.panX+event.clientX-pan.current.x,y:pan.current.panY+event.clientY-pan.current.y}}});return;}if(!move.current.active||!move.current.origin)return;const dx=(event.clientX-move.current.x)/project.view2d.zoom,dy=(event.clientY-move.current.y)/project.view2d.zoom;if(Math.abs(dx)+Math.abs(dy)>.5)move.current.moved=true;preview(moveObjectByPlanDelta(project,move.current.origin,dx,dy));},
  ...
  return <AnyView accessibilityLabel="2D kitchen workspace" accessibilityHint="Drag catalog items here to place them" style={[s.workspace,gridStyle,dropActive&&s.workspaceDropActive]} {...webProps} onClick={(event:any)=>{if(event.target===event.currentTarget)preview({...project,selectedId:undefined});}}>
    <View style={[s.plan,{transform:[{translateX:project.view2d.pan.x},{translateY:project.view2d.pan.y},{scale:project.view2d.zoom}]}]}>{project.objects.map(object=>{const width=Math.max(10,object.widthIn*PLAN_DISPLAY_SCALE),depth=object.kind==='wall'?Math.max(4,object.depthIn*PLAN_DISPLAY_SCALE):Math.max(8,object.depthIn*PLAN_DISPLAY_SCALE);return <AnyPressable key={object.id} accessibilityRole="button" accessibilityLabel={`${object.name}, ${formatInches(object.widthIn)} by ${formatInches(object.depthIn)}`} onPress={()=>preview({...project,selectedId:object.id})} onMouseDown={(event:any)=>{if(event.button!==0||spaceHeld.current)return;event.stopPropagation();move.current={id:object.id,x:event.clientX,y:event.clientY,origin:createObjectDragOrigin(project,object.id),active:true,moved:false,start:project};preview({...project,selectedId:object.id});}} style={[s.object,{left:object.x,top:object.y,width,height:depth,backgroundColor:object.color??(object.kind==='window'?'#91C7DC':object.kind==='door'?'#8A664B':'#C7CECA'),transform:[{rotate:`${object.rotation}deg`}]},project.selectedId===object.id&&s.selected]}><GasRangePlanGraphic object={object}/><Text numberOfLines={1} style={s.label}>{object.name}</Text>{project.view2d.measurements&&<View pointerEvents="none" style={[s.measureWrap,{top:depth+4,minWidth:Math.max(72,width)}]}><View style={s.dimensionLine}/><Text style={s.measureText}>{object.kind==='wall'?formatInches(object.widthIn):`${formatInches(object.widthIn)} × ${formatInches(object.depthIn)}`}</Text></View>}</AnyPressable>;})}</View>
