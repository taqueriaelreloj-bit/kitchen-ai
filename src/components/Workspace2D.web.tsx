...
export function Workspace2D({project,preview,apply,commitHistory}:{project:EditorProject;preview:(project:EditorProject)=>void;apply:Apply;commitHistory:(snapshot:EditorProject,finalProject:EditorProject)=>void}){
  ...
  return <AnyView accessibilityLabel="2D kitchen workspace" accessibilityHint="Drag catalog items here to place them" style={[s.workspace,gridStyle,dropActive&&s.workspaceDropActive]} {...webProps} onClick={(event:any)=>{if(event.target===event.currentTarget)preview({...project,selectedId:undefined});}}>
    <View style={[s.plan,{transform:[{translateX:project.view2d.pan.x},{translateY:project.view2d.pan.y},{scale:project.view2d.zoom}]}]}>{project.objects.map(object=>{
      const width=Math.max(10,planInchesToDisplay(object.widthIn));
      const depth=object.kind==='wall'?Math.max(4,planInchesToDisplay(object.depthIn)):Math.max(8,planInchesToDisplay(object.depthIn));
      const selected=project.selectedId===object.id;
      const rotation=((object.rotation%360)+360)%360;
      const uprightTransform=[{rotate:`${-rotation}deg`},{scale:1/project.view2d.zoom}];
      const showMeasurement=project.view2d.measurements&&(selected||object.kind==='wall');
      return <AnyPressable key={object.id} accessibilityRole="button" accessibilityLabel={`${object.name}, ${formatInches(object.widthIn)} by ${formatInches(object.depthIn)}`} onPress={()=>preview({...project,selectedId:object.id})} onMouseDown={(event:any)=>{if(event.button!==0||spaceHeld.current)return;event.stopPropagation();move.current={id:object.id,x:event.clientX,y:event.clientY,origin:createObjectDragOrigin(project,object.id),active:true,moved:false,start:project};preview({...project,selectedId:object.id});}} style={[s.object,{left:planInchesToDisplay(object.x),top:planInchesToDisplay(object.y),width,height:depth,backgroundColor:object.color??(object.kind==='window'?'#91C7DC':object.kind==='door'?'#8A664B':'#C7CECA'),transform:[{rotate:`${object.rotation}deg`}]},selected&&s.selected]}>
        <GasRangePlanGraphic object={object}/>
        <Text numberOfLines={1} style={[s.label,{transform:uprightTransform}]}>{object.name}</Text>
        {showMeasurement&&<View pointerEvents="none" style={[s.measureWrap,{top:depth+4,minWidth:Math.max(72,width)}]}><View style={s.dimensionLine}/><Text style={[s.measureText,{transform:uprightTransform}]}>{object.kind==='wall'?formatInches(object.widthIn):`${formatInches(object.widthIn)} × ${formatInches(object.depthIn)}`}</Text></View>}
      </AnyPressable>;
    })}</View>
