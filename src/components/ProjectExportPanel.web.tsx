...
function importFile(apply:(project:EditorProject)=>void,onError:(message:string)=>void) {
  const input=document.createElement('input');
  input.type='file'; input.accept='.json,.kitchenai.json,application/json';
  input.onchange=()=>{
    const file=input.files?.[0]; if(!file)return;
    const reader=new FileReader();
    reader.onload=()=>{
      const project=parseProject(String(reader.result??''));
      if(!project){onError('This file is not a valid Kitchen AI project.');return;}
      apply(project);
    };
    reader.onerror=()=>onError('Kitchen AI could not read this project file.');
