...
function CabinetPreview({ item }: { item: DragCatalogItem }) {
  ...
  return <View style={[styles.cabinetPreview, tall && styles.tallPreview, { backgroundColor: item.previewColor }]}>
    {drawers
...
function RefrigeratorPreview({ item }: { item: DragCatalogItem }) {
  ...
  if (retro) return <View style={[styles.retroPreview, { backgroundColor: item.previewColor }]}>
    <View style={styles.retroFreezer}/><View style={styles.retroSeparator}/><View style={styles.retroDoor}/>
    <View style={styles.retroHandleTop}/><View style={styles.retroHandleBottom}/>
  </View>;
...
function RangePreview({ item }: { item: DragCatalogItem }) {
  ...
  return <View style={[styles.rangePreview, { backgroundColor: item.previewColor }]}>
    <View style={styles.rangeCooktop}>
      {[0, 1, 2, 3].map(index => <View key={index} style={[styles.rangeBurner, index % 2 ? styles.burnerRight : styles.burnerLeft, index > 1 && styles.burnerBottom]}/>)}
      {griddle && <View style={styles.rangeGriddle}/>} 
    </View>
...
const hasCatalogType = (transfer?: DataTransfer | null) => Boolean(transfer && Array.from(transfer.types).includes(KITCHEN_CATALOG_DRAG_MIME));

export function CatalogHoverMenu({ project, apply, preview }: Props) {
  const { width } = useWindowDimensions();
  const compact = width < 1120;
  const veryNarrow = width < 760;
  const flyoutWidth = Math.min(612, Math.max(330, width - 24));
  const cardWidth: number | string = flyoutWidth < 430 ? '100%' : flyoutWidth < 610 ? '48%' : 190;
  ...
  useEffect(() => {
    const workspace = () => document.querySelector('[aria-label="2D kitchen workspace"]') as HTMLElement | null;
    const insideWorkspace = (event: DragEvent, element: HTMLElement) => {
      const rect = element.getBoundingClientRect();
      return event.clientX >= rect.left && event.clientX <= rect.right && event.clientY >= rect.top && event.clientY <= rect.bottom;
    };
    const onDragOver = (event: DragEvent) => {
      if (!hasCatalogType(event.dataTransfer)) return;
      const element = workspace();
      if (!element || !insideWorkspace(event, element)) { setOverWorkspace(false); return; }
    ...
    const onDrop = (event: DragEvent) => {
      if (!hasCatalogType(event.dataTransfer)) return;
      const element = workspace();
      if (!element || !insideWorkspace(event, element)) return;
      const serialized = event.dataTransfer?.getData(KITCHEN_CATALOG_DRAG_MIME) || event.dataTransfer?.getData('text/plain');
      const item = parseDragCatalogItem(serialized);
      if (!item) return;
      event.preventDefault();
      event.stopPropagation();
      const rect = element.getBoundingClientRect();
      const point = workspaceClientPointToPlan(project, { x: event.clientX, y: event.clientY }, { x: rect.left, y: rect.top });
      apply(addDragCatalogItemAtPlanPoint(project, item.id, point));
    ...
    const onDragEnd = () => setOverWorkspace(false);
    ...
    <View style={styles.bar}>
      ...
      <AnyView style={styles.categories}>
        {DRAG_CATALOG_CATEGORIES.map((category, index) => {
          ...
          return <AnyView key={category.id} style={styles.categoryWrap} onMouseEnter={() => open(category.id)} onMouseLeave={scheduleClose}>
            <Pressable
            ...
            {active && <View style={[styles.flyout, { width: flyoutWidth }, alignRight && styles.flyoutRight]}>
              ...
              <ScrollView style={styles.flyoutScroll} contentContainerStyle={styles.itemGrid} showsVerticalScrollIndicator>
                {category.items.map(item => <HtmlCard
                  key={item.id}
                  role="button"
                  tabIndex={0}
                  draggable={true}
                  ...
                  style={StyleSheet.flatten([styles.itemCard, { width: cardWidth }, draggingId === item.id && styles.itemDragging]) as any}
                >
      ...
      {draggingId && <View style={[styles.dragState, overWorkspace && styles.dragStateReady]}><Text style={[styles.dragStateText, overWorkspace && styles.dragStateTextReady]}>{overWorkspace ? 'Release to place' : 'Drag into the 2D plan'}</Text></View>}
    </View>
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  itemCard: { minHeight: 215, boxSizing: 'border-box', borderRadius: 10, borderWidth: 1, borderStyle: 'solid', borderColor: '#C2CECA', backgroundColor: '#FFFFFF', padding: 8, cursor: 'grab', outline: 'none' } as any,
  itemDragging: { opacity: .55, borderColor: '#147356', backgroundColor: '#E6F3EF', cursor: 'grabbing' } as any,
  ...
  dragState: { position: 'absolute', right: 12, top: 8, borderRadius: 999, backgroundColor: '#F1E5B7', paddingHorizontal: 12, paddingVertical: 7, zIndex: 160 },
  dragStateReady: { backgroundColor: '#D8F1E8' },
  dragStateText: { fontSize: 10, fontWeight: '900', color: '#6B5619' },
  dragStateTextReady: { color: '#155B46' },
