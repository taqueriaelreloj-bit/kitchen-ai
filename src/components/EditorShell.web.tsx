...
export function EditorShell({initialProject,onProjectChange,onExit}:{initialProject:EditorProject;onProjectChange:(project:EditorProject)=>void;onExit:()=>void}) {
  ...
  return <View style={[s.app,WEB_VIEWPORT_STYLE]}>
    ...
    <View style={s.main}>
      ...
      {!maximized&&<View style={[s.leftPanel,compact&&s.leftPanelCompact,narrow&&s.leftPanelNarrow]}>{checkOpen?<LayoutCheckPanel project={project} apply={apply}/>:aiOpen?<AIDesignPanel project={project} apply={next=>apply(next)}/>:<ToolPanel tool={tool} project={project} selected={selected} apply={apply} onProjectOpen={openImportedProject} onHome={onExit}/>}</View>}
      <View style={s.center}>
