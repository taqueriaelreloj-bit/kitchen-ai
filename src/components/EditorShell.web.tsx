...
export function EditorShell({initialProject,onProjectChange,onExit}:{initialProject:EditorProject;onProjectChange:(project:EditorProject)=>void;onExit:()=>void}) {
  ...
  const toolPanelVisible=!maximized&&(aiOpen||checkOpen||tool!=='Plan');
  const propertiesDocked=rightOpen&&!compact;
  const editorViewport=()=>({
    width:Math.max(420,viewportWidth-(maximized?24:toolbarWidth+(toolPanelVisible?leftPanelWidth:0)+(propertiesDocked?PROPERTY_PANEL_WIDTH:0)+24)),
  ...
  const open2D=()=>{
    ...
    const next={...project,viewMode:'2d' as const};
    preview({...next,view2d:fit2DView(next,editorViewport())});
  };
  const open3D=()=>{
  ...
  const chooseTool=(next:Tool)=>{setAiOpen(false);setCheckOpen(false);setTool(current=>current===next&&next!=='Plan'?'Plan':next);};
  ...
  useEffect(()=>{
    const handle=(event:KeyboardEvent)=>{
      ...
      if(!command&&!event.altKey&&key==='2'){open2D();return;}
  ...
  return <View style={[s.app,WEB_VIEWPORT_STYLE]}>
    ...
    <View style={s.main}>
      ...
      {!maximized&&rightOpen&&!compact&&<View style={s.desktopProperties}><ContextPanel project={project} selected={selected} apply={next=>apply(next)}/></View>}
...
const s=StyleSheet.create({
  ...
  quickTransform:{position:'absolute',right:12,bottom:12,zIndex:20},
  quickTransform3d:{position:'absolute',right:12,bottom:12,zIndex:20},
  ...
  desktopProperties:{width:PROPERTY_PANEL_WIDTH,minWidth:PROPERTY_PANEL_WIDTH,maxWidth:PROPERTY_PANEL_WIDTH,flexGrow:0,flexShrink:0,minHeight:0,overflow:'hidden'},
  propertiesOverlay:{position:'absolute',right:10,top:10,bottom:10,width:330,zIndex:70,borderRadius:12,borderWidth:1,borderColor:'#AABAB4',backgroundColor:'#F7F9F8',overflow:'hidden',shadowColor:'#000000',shadowOpacity:.24,shadowRadius:18,shadowOffset:{width:0,height:8},elevation:24},
  ...
  propertiesOverlayBody:{flex:1,minHeight:0,alignItems:'stretch',overflow:'hidden'},
});
