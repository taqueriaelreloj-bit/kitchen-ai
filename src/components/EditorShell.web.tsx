...
import { clampZoom, EditorProject, reset2DView, reset3DView } from '../domain/editor';
...
export function EditorShell({initialProject,onProjectChange,onExit}:{initialProject:EditorProject;onProjectChange:(project:EditorProject)=>void;onExit:()=>void}) {
  ...
  return <View style={s.app}>
    <View style={s.top}>
      <View style={s.brand}><Text style={s.logo}>K</Text><View><Text style={s.brandTitle}>Kitchen AI</Text><Text style={s.projectName}>{project.name}</Text></View></View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.topActions}>
        <Button label="Undo" disabled={!undo.current.length} onPress={doUndo}/><Button label="Redo" disabled={!redo.current.length} onPress={doRedo}/>
        <Button label="Save" onPress={()=>onProjectChange(project)}/><Button label="Open Project" onPress={()=>setTool('Project')}/>
        <Button label="2D" active={project.viewMode==='2d'} onPress={()=>preview({...project,viewMode:'2d'})}/><Button label="3D" active={project.viewMode==='3d'} onPress={()=>preview({...project,viewMode:'3d'})}/>
        <Button label="Fit View" onPress={fit}/><Button label="Export" onPress={()=>setTool('Export')}/><Button label="Estimate" onPress={onExit}/><Button label="Help" onPress={()=>setTool('View')}/><Button label="Settings" onPress={()=>setTool('Settings')}/>
      </ScrollView>
    </View>
    <View style={s.main}>
      {!maximized&&<ScrollView style={s.toolbar}>{TOOLS.map(item=><ToolButton key={item} tool={item} active={tool===item} onPress={()=>setTool(item)}/>)}</ScrollView>}
      {!maximized&&<View style={s.leftPanel}><ToolPanel tool={tool} project={project} selected={selected} apply={apply} onHome={onExit}/></View>}
      <View style={s.center}>
        <View style={s.workspaceHeader}><Text style={s.workspaceTitle}>{project.viewMode==='2d'?'2D Plan':'3D WebGL Kitchen'}</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.headerActions}>{project.viewMode==='2d'&&<><Button label="Zoom −" onPress={()=>preview({...project,view2d:{...project.view2d,zoom:clampZoom(project.view2d.zoom-.1)}})}/><Text style={s.zoom}>{Math.round(project.view2d.zoom*100)}%</Text><Button label="Zoom +" onPress={()=>preview({...project,view2d:{...project.view2d,zoom:clampZoom(project.view2d.zoom+.1)}})}/></>}<Button label="Fit" onPress={fit}/><Button label="Reset" onPress={reset}/><Button label="Fullscreen" onPress={fullscreen}/><Button label={maximized?'Restore':'Maximize'} onPress={()=>setMaximized(!maximized)}/><Button label={rightOpen?'Hide Properties':'Properties'} onPress={()=>setRightOpen(!rightOpen)}/></ScrollView></View>
        {project.viewMode==='2d'?<Workspace2D project={project} preview={preview} commitHistory={commitHistory}/>:<View style={s.workspace}><WebGLViewport project={project} preview={preview}/></View>}
        <View style={s.status}><Text style={s.statusText}>{project.viewMode==='3d'?'WebGL 3D · real openings · countertop materials · continuous toe kick':`${Math.round(project.view2d.zoom*100)}% · ${project.view2d.snap?'Snap On':'Snap Off'}`}</Text><Text style={s.statusText}>{project.viewMode==='3d'?'Wheel Zoom · Drag Orbit · Middle/Right Pan · Double-click Focus':'Wheel Zoom · Middle/Space Pan · Drag = one Undo step'}</Text></View>
      </View>
      {!maximized&&rightOpen&&<ContextPanel project={project} selected={selected} apply={next=>apply(next)}/>} 
    </View>
  </View>;
}
