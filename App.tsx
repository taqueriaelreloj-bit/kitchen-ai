...
import { reconstructRoom, REQUIRED_SCAN_ANGLES, roomArea } from './src/domain/room';
...
function Reconstruct({ photos, done }: { photos: ScanPhoto[]; done: (r: RoomModel) => void }) {
  useEffect(() => { const id = setTimeout(() => done(reconstructRoom(photos)), 1400); return () => clearTimeout(id); }, [photos, done]);
...
function RoomReview({ room, back, next }: { room: RoomModel; back: () => void; next: () => void }) {
  return <Screen><StepHeader step="Paso 2 de 5" title="Revise su cocina" onBack={back} /><ScrollView contentContainerStyle={s.content}>
    <View style={s.floorPlan}><View style={s.planInner}><Text style={s.planLabel}>{room.widthM} m</Text><View style={s.planWindow} /><Text style={s.planArea}>{roomArea(room)} m²</Text><Text style={s.planLength}>{room.lengthM} m</Text></View></View>
    <View style={s.card}><Text style={s.cardTitle}>Modelo encontrado</Text><Text style={s.rowText}>Forma en L  ·  Techo {room.heightM} m</Text><Text style={s.rowText}>1 puerta  ·  1 ventana</Text><Text style={s.confidence}>✓ Buena calidad de escaneo</Text></View>
    <Text style={s.note}>Las medidas son aproximadas en este MVP. Confírmelas antes de comprar materiales.</Text><PrimaryButton label="Sí, crear diseños" onPress={next} />
  </ScrollView></Screen>;
}
...
function KitchenApp() {
  ...
  return <SafeAreaView style={s.safe} edges={stage === 'scan' ? [] : ['top', 'bottom']}>
    ...
    {stage === 'room' && room && <RoomReview room={room} back={() => setStage('scan')} next={() => { setDesigns(generateDesigns(room)); setStage('designs'); }} />}
