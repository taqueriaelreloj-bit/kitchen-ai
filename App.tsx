...
function Home({saved,start,resume}:{saved:boolean;start:()=>void;resume:()=>void}){
  return <Screen><ScrollView contentContainerStyle={s.home}>
    ...
    <View style={s.flow}>{['1  Escanee su cocina','2  Elija un diseño','3  Edite en 2D y 3D'].map(item=><Text key={item} style={s.flowItem}>{item}</Text>)}</View>
...
function Scan({done,back}:{done:(photos:ScanPhoto[])=>void;back:()=>void}){
...
function KitchenApp(){
  ...
  return <SafeAreaView style={s.safe} edges={stage==='scan'?[]:['top','bottom']}>
    {stage==='home'&&<Home saved={saved} start={()=>{setEditorProject(undefined);setStage('scan');}} resume={resume}/>} 
    {stage==='scan'&&<Scan back={()=>setStage('home')} done={captured=>{setPhotos(captured);setStage('reconstruct');}}/>}
