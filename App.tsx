...
type Stage = 'home' | 'scan' | 'reconstruct' | 'room' | 'designs' | 'customize' | 'editor' | 'price';
...
function Home({ saved, start, resume }: { saved: boolean; start: () => void; resume: () => void }) {
  return <Screen><ScrollView contentContainerStyle={s.home}>
    ...
    <Text style={s.lead}>Escanee el cuarto y termine el diseño en un editor 2D/3D profesional y fácil de usar.</Text>
    <View style={s.flow}>{['1  Escanee su cocina', '2  Elija un diseño', '3  Edite en 2D y 3D'].map(x => <Text key={x} style={s.flowItem}>{x}</Text>)}</View>
...
function KitchenApp() {
  ...
  async function resume() {
    try {
      ...
      setEditorProject(project); setRoom(project.room); setDesign(project.design); setStage('editor');
  ...
  if (stage === 'editor' && editorProject) return <SafeAreaView style={s.editorSafe} edges={['top','bottom']}><EditorShell initialProject={editorProject} onProjectChange={setEditorProject} onExit={() => setStage('price')} /></SafeAreaView>;
  return <SafeAreaView style={s.safe} edges={stage === 'scan' ? [] : ['top', 'bottom']}>
    ...
    {stage === 'price' && room && design && <Price room={room} design={design} back={() => setStage(editorProject ? 'editor' : 'customize')} home={() => setStage('home')} />}
  </SafeAreaView>;
}
