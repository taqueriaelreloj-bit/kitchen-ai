...
CATALOG = [
    ...
    ("tall-fridge-surround-36x84", "tall", "fridge-surround", 36, 24, 84, 0),
...
def build_cabinet(kind, root, col, m, w, d, h):
    is_base = kind in {"base", "drawers", "sink", "corner", "tall", "oven-tower", "fridge-surround"}
    cabinet_shell(root, col, m, w, d, h, is_base)
    bottom = s(TOE_H_IN) if is_base else 0
    add_face_frame(root, col, m, w, d, h, bottom, h)

    if kind == "base":
        add_shelf(root, col, m, w, d, bottom + (h - bottom) * .46)
        drawer_h = s(6.25)
        ...
        heights = [usable * .24, usable * .38, usable * .38]
        current = bottom + s(1.25)
        for i, fh in enumerate(heights, 1):
            z = current + fh / 2
            add_drawer_box(root, col, m, w, d, z, fh - s(.15), i)
            current += fh
